...
parser.add_argument('--feature', default='noname', type=str,help='featurename' )
...
def extract_feature(model,dataloaders):
    ...
    for data in dataloaders:
        ...
        for i in range(2):
            ...
            # g= make_dot( outputs, params=dict(list(model.named_parameters()) + [('x', input_img)]))
            # print (type(g))
            # g.save('res.png')
...
gallery_cam,gallery_label,gallery_names = get_id(gallery_path)
query_cam,query_label,query_names = get_id(query_path)

######################################################################
# Load Collected data Trained model
print('-------test-----------')
if opt.use_dense:
    model_structure = ft_net_dense(484)
else:
    model_structure = ft_net(485)

if opt.PCB:
    model_structure = PCB(483)
...
# scipy.io.savemat("cross"+name+feature+'pytorch_result.mat',result)
